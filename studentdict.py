...
def searchstu(studentadd):
	...
	for i in range(len(studentadd)):
		print("|{:<10}|{:<10}|{:<10}|{:<10}".format(i+1,studentadd[i].get('name'),studentadd[i].get('age'),studentadd[i].get('classid')))
...
def addstu(studentadd):
	...
	student['classid']=input("请输入学员的班级号:")
...
chose=main()
studentadd=[{'name':'zhangsan','age':20,'classid':'python01'},{'name':'lisi','age':22,'classid':'python02'}]
